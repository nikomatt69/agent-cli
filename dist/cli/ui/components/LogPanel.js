...
class LogPanel extends events_1.EventEmitter {
    constructor(parent, config, logger) {
        super();
        this.logs = [];
        this.filteredLogs = [];
        this.currentFilter = '';
        this.logLevels = new Set(['debug', 'info', 'warn', 'error']);
        this.focused = false;
        this.maxLogs = 1000;
        this.config = config;
        this.logger = logger;
    ...
    createComponents(parent) {
        ...
        // Control bar (top)
        this.controlBar = blessed_1.default.box({
        ...
        // Filter input
        this.filterInput = blessed_1.default.textbox({
        ...
        // Log display
        this.logDisplay = blessed_1.default.box({
            parent: this.container,
            top: 3,
            left: 0,
            width: '100%',
    ...
    createControlButtons() {
        const clearButton = blessed_1.default.button({
            parent: this.controlBar,
            top: 0,
            left: '52%',
            width: 8,
        ...
        const exportButton = blessed_1.default.button({
            parent: this.controlBar,
            top: 0,
            left: '61%',
            width: 10,
        ...
        const levelButton = blessed_1.default.button({
            parent: this.controlBar,
            top: 0,
            right: 1,
            width: 12,
    ...
    setupEventHandlers() {
        ...
        this.filterInput.on('keypress', (ch, key) => {
            if (key.name === 'escape') {
                this.clearFilter();
            }
            else {
        ...
        // Log display events
        this.logDisplay.on('keypress', (ch, key) => {
    ...
    applyFilter() {
        ...
        if (this.currentFilter.trim()) {
            ...
            filtered = filtered.filter(log => log.message.toLowerCase().includes(filter) ||
    ...
    toggleLogLevels() {
        ...
        const currentIndex = levelSets.findIndex(set => set.size === this.logLevels.size &&
