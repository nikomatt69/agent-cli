...
class NikCLI {
    ...
    async executeAgent(name, task, options) {
        ...
        try {
            ...
            const availableAgents = Object.keys(AGENT_CAPABILITIES);
            ...
            const result = await modernAgentOrchestrator.executeTask(name, task);
    ...
    async autoExecute(task, options) {
        ...
        try {
            if (options.planFirst) {
                ...
                const suggestedAgents = modernAgentOrchestrator.suggestAgent(task);
                const bestAgent = suggestedAgents[0] || 'full-stack-developer';
                const result = await modernAgentOrchestrator.executeTask(bestAgent, task);
    ...
    async showStatus() {
        ...
        console.log(`  Model: ${chalk_1.default.green(advancedAIProvider.getCurrentModelInfo().name)}`);
        // Agent info
        if (this.currentAgent) {
    ...
    async listAgents() {
        ...
        Object.entries(AGENT_CAPABILITIES).forEach(([name, capability]) => {
            ...
            console.log(`  ${chalk_1.default.dim('Examples:')} ${capability.examples.slice(0, 2).join(', ')}`);
            console.log();
        });
    }
    /**
    ...
    async listModels() {
        ...
        const currentModel = advancedAIProvider.getCurrentModelInfo().name;
    ...
    switchModel(modelName) {
        try {
            ...
            simpleConfigManager.setCurrentModel(modelName);
    ...
    showChatWelcome() {
        const title = gradient_string_1.default.rainbow('🤖 NikCLI');
        ...
        console.log((0, boxen_1.default)(`${title}\n${subtitle}\n\n` +
            ...
            `${chalk_1.default.blue('Model:')} ${chalk_1.default.green(advancedAIProvider.getCurrentModelInfo().name)}\n` +
