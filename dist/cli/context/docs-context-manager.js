...
class DocsContextManager {
    ...
    async optimizeContext() {
        ...
        if (stats.utilizationPercent > 90) {
            ...
            const docs = this.getLoadedDocs();
            const toRemove = Math.ceil(docs.length * 0.2);
    ...
    compressLoadedDocs() {
        for (const [id, doc] of this.loadedDocs) {
            if (doc.content.length > 5000) {
                ...
                const important = paragraphs.filter(p => p.includes('```') ||
                    p.length < 500 ||
                    p.includes('##') ||
                    /\b(important|note|warning|example)\b/i.test(p));
