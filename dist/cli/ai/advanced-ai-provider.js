        ...
        return {
            ...
            write_file: (0, ai_1.tool)({
                ...
                parameters: zod_1.z.object({
                    ...
                    validate: zod_1.z.boolean().default(true).describe('Use LSP validation before writing'),
                    agentId: zod_1.z.string().optional().describe('ID of the agent making this request'),
                    reasoning: zod_1.z.string().optional().describe('Reasoning behind this file creation/modification'),
                }),
                execute: async ({ path, content, backup, validate, agentId, reasoning }) => {
                    try {
                        ...
                        if (validate) {
                            console.log(chalk_1.default.cyan(`🔍 Validating ${path} with LSP before writing...`));
                            ...
                            if (!validationResult.isValid) {
                                if (validationResult.fixedContent) {
                                    ...
                                    console.log(chalk_1.default.green(`🔧 Auto-fix and formatting applied successfully`));
                                }
                                else {
                                    return {
                                        success: false,
                                ...
                                if (validationResult.formatted) {
                                    console.log(chalk_1.default.green(`✅ File formatted and validated successfully`));
                                }
                                else {
                                    console.log(chalk_1.default.green(`✅ Validation passed for ${path}`));
                                }
                            }
                        }
                        let backedUp = false;
                        ...
                        (0, fs_1.writeFileSync)(fullPath, finalContent, 'utf-8');
                        const stats = (0, fs_1.statSync)(fullPath);
                        console.log(chalk_1.default.green(`✅ File written successfully: ${path} (${stats.size} bytes)`));
                        this.executionContext.set(`file:${path}`, {
                            content,
                            stats,
...
- Ensure code is production-ready`;
    ...
    detectProjectType(workingDirectory) {
        try {
            const packageJsonPath = (0, path_1.join)(workingDirectory, 'package.json');
            const cargoTomlPath = (0, path_1.join)(workingDirectory, 'Cargo.toml');
            const requirementsPath = (0, path_1.join)(workingDirectory, 'requirements.txt');
            const goModPath = (0, path_1.join)(workingDirectory, 'go.mod');
            if ((0, fs_1.existsSync)(packageJsonPath)) {
                const packageJson = JSON.parse((0, fs_1.readFileSync)(packageJsonPath, 'utf-8'));
            ...
            if ((0, fs_1.existsSync)(cargoTomlPath)) {
                return 'rust';
            }
