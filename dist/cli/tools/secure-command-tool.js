...
class SecureCommandTool {
    ...
    async createBatchSession(commands, options = {}) {
        ...
        if (approved) {
            console.log(chalk_1.default.green(`✅ Batch session approved: ${sessionId}`));
            console.log(chalk_1.default.blue('🚀 Ready for async execution'));
        }
        else {
            console.log(chalk_1.default.yellow('❌ Batch session not approved'));
        }
        return session;
    }
    /**
