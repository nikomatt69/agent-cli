...
<Tabs>
  ...
  <Tab title="Advanced Options">
    ...
    /run --cwd=/path/to/project npm test
    ...
    # Execute with approval bypass (developer mode only)
    /run --force rm -rf node_modules
    ```
  </Tab>
...
<Tabs>
  ...
  <Tab title="Set Variables">
    ...
    /env NODE_ENV=production API_URL=https://prod.api.com
...
<Tabs>
  ...
  <Tab title="Conditional Execution">
    ...
    /exec --if-exists package.json npm install
    /exec --if-not-exists node_modules npm install
    /exec --if-command-exists docker docker ps
    
    # Execute with retry logic
    /exec --retry 3 npm run flaky-test
    /exec --retry-delay 5 curl https://api.example.com
    ```
  </Tab>
  ...
  <Tab title="Pipeline Execution">
    ...
    # Pipe commands
    /exec "cat package.json | jq '.dependencies'"
    
    # Complex shell operations
    /exec "find src -name '*.ts' | xargs grep -l 'TODO' | head -5"
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="High Risk">
    **Requires Explicit Approval:**
    ...
    network operations (curl, wget with write)
    database operations
...
<Tabs>
  ...
  <Tab title="Output Processing">
    ```bash
    # Filter output
    /run --grep "error" npm run build
    /run --filter "warning" npm test
    
    # Limit output lines
    /run --head 20 npm run build
    /run --tail 50 docker logs container
    
    # Save output to file
    /run --save build.log npm run build
    /run --append test.log npm test
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Output Management">
    ...
    /run --buffer-size 8192 npm run build
    /run --compress-output docker build .
    /run --stream-threshold 1000 npm test
    ```
  </Tab>
  ...
  <Tab title="Process Management">
    ...
    /process --auto-restart dev-server
    /process --health-check api-server
    /process --resource-limits background-job
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Output Buffer Issues">
    **Problem:** Large output causes memory or display issues
