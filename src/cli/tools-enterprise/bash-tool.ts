...
class CommandAnalyzer {
  ...
  analyzeSafety(command: string): { level: 'SAFE' | 'CAUTION' | 'DANGER', reasons: string[] } {
  ...
  analyzeResourceImpact(command: string): 'LOW' | 'MEDIUM' | 'HIGH' {
  ...
  extractPotentialFilePaths(command: string): string[] {
    const paths: string[] = [];
...
function generateContextualBashPrompt(ctx: Context): string {
...
export const EnterpriseBashtool = define('enterprise-bash', {
  category: ToolCategory.DEVELOPMENT,
  version: '2.0.0',
  tags: ['bash', 'shell', 'terminal', 'execution', 'enterprise'],
  requiredPermissions: [Permission.EXECUTE_COMMANDS, Permission.READ_FILES],
  contextDependencies: [],
  ...
  init: async () => ({
    ...
    parameters: z.object({
      command: z.string().describe('The bash command to execute with full safety analysis'),
      timeout: z.number().optional().describe('Optional timeout in milliseconds (max 600000ms)'),
      mode: z.enum(['dry-run', 'safe', 'interactive', 'force']).optional().default('safe').describe('Execution mode with different safety levels'),
      description: z.string().describe('Clear, concise description of what this command accomplishes (5-15 words)'),
      workingDirectory: z.string().optional().describe('Optional working directory override'),
      environmentVariables: z.record(z.string()).optional().describe('Optional environment variables'),
    }),
    ...
    async execute(args, ctx): Promise<ToolResult<BashMetadata>> {
