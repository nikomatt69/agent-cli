...
export class UnifiedChatInterface extends EventEmitter {
  ...
  private async handleSlashCommand(command: string): Promise<void> {
    ...
    switch (cmd.toLowerCase()) {
      ...
      case 'plan':
        ...
        console.log(chalk.green(`📋 Plan mode: ${this.session.planMode ? 'ON' : 'OFF'}`));
        break;
      case 'status':
        this.showStatus();
        break;
  ...
  private showHelp(): void {
    const helpBox = boxen(
      ...
      chalk.green('/plan') + chalk.gray('     - Toggle plan mode (currently: ') + (this.session.planMode ? chalk.green('ON') : chalk.red('OFF')) + chalk.gray(')\n') +
      ...
      chalk.yellow('💡 Just type your request and I\'ll help you!'),
      {
        padding: 1,
        borderStyle: 'round',
        borderColor: 'cyan'
