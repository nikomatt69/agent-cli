...
export class VMOrchestrator extends EventEmitter {
  ...
  async *executeCommandStreaming(containerId: string, command: string): AsyncGenerator<CommandStreamChunk, void, unknown> {
    try {
      ...
      // For now, execute command normally and yield result
      // In a full implementation, this would stream command output line by line
      const result = await this.containerManager.executeCommand(containerId, command);

      // Yield command output chunks (simulated)
