...
export class NikCLI {
    ...
    private async handleSlashCommand(command: string): Promise<void> {
        ...
        try {
            switch (cmd) {
                ...
                case 'create-agent':
                    if (args.length === 0) {
                        console.log(chalk.red('Usage: /create-agent [--vm|--container] <spec>'));
                        console.log(chalk.gray('Use --vm flag for VM-based agents'));
                        return;
                    }
    ...
    /**
     * VM mode: Chat directly with VM agents in containers
     */
    private async handleVMMode(input: string): Promise<void> {
        console.log(chalk.blue('🐳 VM Mode: Communicating with VM agents...'));

        try {
            // Get active containers from SlashCommandHandler's VMOrchestrator
            ...
            // Send input to all active VM containers
            console.log(chalk.cyan(`📡 Sending to ${containers.length} VM agent(s):`));
            console.log(chalk.gray(`Input: ${input.substring(0, 100)}${input.length > 100 ? '...' : ''}`));

            for (const container of containers) {
                try {
                    console.log(chalk.blue(`🤖 VM Agent ${container.id.slice(0, 8)}: Processing...`));

                    // In a real implementation, this would send the input to the VM agent
                    // For now, we'll simulate a response
                    await new Promise(resolve => setTimeout(resolve, 1000));

                    console.log(chalk.green(`✅ VM Agent ${container.id.slice(0, 8)}: Task completed`));
    ...
    private async handleAdvancedFeatures(command: string, args: string[]): Promise<void> {
        try {
            switch (command) {
                ...
                case 'factory': {
                    const result = await AgentCommands.factory(args);
                    if (result.success) {
                        if (result.data) {
                ...
                case 'create-agent': {
                    const result = await AgentCommands.createAgent(args);
                    if (result.success) {
                        if (result.data) {
                            console.log(JSON.stringify(result.data, null, 2));
                        } else {
                ...
                case 'launch-agent': {
                    const result = await AgentCommands.launchAgent(args);
                    if (result.success) {
                        if (result.data) {
                            console.log(JSON.stringify(result.data, null, 2));
                        } else {
                ...
                case 'list-agents': {
                    const result = await AgentCommands.listAgents(args);
                    if (result.success) {
                        if (result.data) {
                            console.log(JSON.stringify(result.data, null, 2));
                        } else {
                ...
                case 'describe-agent': {
                    const result = await AgentCommands.describeAgent(args);
                    if (result.success) {
                        if (result.data) {
                            console.log(JSON.stringify(result.data, null, 2));
                        } else {
                ...
                case 'pause-agent': {
                    const result = await AgentCommands.pauseAgent(args);
                    if (result.success) {
                        if (result.data) {
                            console.log(JSON.stringify(result.data, null, 2));
                        } else {
                ...
                case 'resume-agent': {
                    const result = await AgentCommands.resumeAgent(args);
                    if (result.success) {
                        if (result.data) {
                            console.log(JSON.stringify(result.data, null, 2));
                        } else {
                ...
                case 'kill-agent': {
                    const result = await AgentCommands.killAgent(args);
                    if (result.success) {
                        if (result.data) {
                            console.log(JSON.stringify(result.data, null, 2));
                        } else {
...
`;
     ...
     * Initialize unified agent system
     */
    private async initializeUnifiedAgentSystem(): Promise<void> {
        try {
            // Initialize agent persistence
            await agentPersistence.initialize();
            
            // Unified agent factory is already initialized in constructor
            console.log(chalk.dim('   Unified agent system ready'));
        } catch (error: any) {
            console.log(chalk.yellow(`⚠️ Unified agent system initialization failed: ${error.message}`));
        }
    }
