...
export class NikCLI {
    ...
    constructor() {
        ...
        // Initialize VM integration system (lazy loading)
        this.initializeVMIntegration();
    ...
    private async handleDefaultMode(input: string): Promise<void> {
        ...
        const analysisMatch = input.match(/analizza\s+(?:la\s+)?repository\s+(.+)/i);
        if (analysisMatch) {
            const repositoryUrl = analysisMatch[1].trim();
            console.log(chalk.blue(`🔍 Repository analysis requested: ${repositoryUrl}`));
            
            try {
                // Import VM integration if not already imported
                const { vmIntegration } = await import('./core/vm-integration');
                
                // Initialize VM integration if not already done
                if (!vmIntegration.getStatus().enabled) {
                    await vmIntegration.initialize();
                }
                
                // Start repository analysis
                const results = await vmIntegration.handleRepositoryAnalysis(repositoryUrl, input);
                
                console.log(chalk.green('✅ Repository analysis completed!'));
                console.log(chalk.cyan('📋 Check the generated pull request for detailed results.'));
                
                return;
            } catch (error: any) {
                console.log(chalk.red(`❌ Repository analysis failed: ${error.message}`));
                return;
            }
        }
...
`;
     ...
     * Initialize VM integration system (lazy loading)
     */
    private async initializeVMIntegration(): Promise<void> {
        try {
            // Import VM integration dynamically to avoid startup issues
            const { vmIntegration } = await import('./core/vm-integration');
            
            // Initialize VM integration in background
            vmIntegration.initialize().catch((error: any) => {
                console.debug('VM Integration initialization failed:', error.message);
            });

            console.log(chalk.dim('🐳 VM Integration system ready (lazy loading)'));
        } catch (error: any) {
            console.debug('VM Integration not available:', error.message);
        }
    }
}
