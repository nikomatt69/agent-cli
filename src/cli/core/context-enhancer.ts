...
export class ContextEnhancer {
  ...
  private getEnhancedSystemPrompt(context: EnhancementContext): string {
...
Respond in a helpful, professional manner with clear explanations and actionable insights.`;
  ...
  private isFileRelatedQuery(content: any): boolean {
  ...
  private getWorkspaceContext(workingDirectory: string): string {
    try {
      const files = readdirSync(workingDirectory, { withFileTypes: true });
...
- Main directories: ${files.filter(f => f.isDirectory()).map(f => f.name).slice(0, 5).join(', ')}`;
  ...
  private analyzeFileTypes(files: any[], workingDirectory: string): Record<string, number> {
  ...
  private getConversationMemory(memory: CoreMessage[]): string {
  ...
  private getExecutionContext(executionContext: Map<string, any>): string {
